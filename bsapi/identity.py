...
class Identity:
    ...
    def from_json(name: str, json_obj: any):
        return Identity(name, json_obj['userId'], json_obj['userKey'], set(json_obj['defaultFor']))
...
class IdentityStore:
    ...
    def store_path(self) -> str:
        ...
        """
        default_path = os.path.join(os.path.expanduser('~'), '.config', 'brightspace', 'identities.json')
        ...
        if os.path.isfile(path):
            with open(path, 'r', encoding='utf-8') as f:
                ...
                for host, identities in json_obj['identities'].items():
                    self.identities[host] = {name: Identity.from_json(name, identity) for name, identity in
                                             identities.items()}
    ...
    def store(self, path: str = None):
        ...
        for host, identities in self.identities.items():
            json_obj['identities'][host] = {identity.name: identity.to_json() for identity in identities.values()}
...
class TablePrinter:
    ...
    def add_column(self, name: str, column_align: str = ALIGN_CENTER, row_align: str = ALIGN_LEFT):
...
class IdentityManager:
    ...
    def __init__(self, host: str, app_id: str = None, app_key: str = None, client_callback_url: str = None,
                 store_path: str = None):
    ...
    def from_config(config: bsapi.APIConfig):
        ...
        """
        return IdentityManager(config.lms_url, config.app_id, config.app_key, config.client_app_url)
    ...
    def load_store(self) -> bool:
        ...
        try:
            ...
            print(f'[ERROR] failed to load identity store from path "{store_path}" due to exception: {e}')
            return False
    ...
    def save_store(self) -> bool:
        ...
        try:
            ...
            store_path = self.store_path if self.store_path else self.store.store_path()
            print(f'[ERROR] failed to save identity store to path "{store_path}" due to exception: {e}')
            return False
    ...
    def _list_identities(self):
        ...
        table.add_rows([[id_.name, id_.user_id, ', '.join(sorted(id_.default_for))] for id_ in identities])
    ...
    def _print_help_manager():
        ...
        print('* test <name>          - test identity with <name> using Brightspace API')
    ...
    def _complete_add_identity_auth(self, user_id: str, user_key: str) -> bool:
        context = bsapi.APIContext(self.app_id, self.app_key, user_id, user_key, self.host)
        ...
        try:
            ...
            description = f'{whoami.first_name} {whoami.last_name} ({whoami.unique_name})'

            print()
            print(f'Identified as: {description}')
            print(f'Enter a name for the new identity; leave blank to use suggested "{suggested_name}"')
    ...
    def _add_identity_auth(self) -> bool:
        ...
        if not self.client_callback_url:
            print('[ERROR] client_callback_url not specified; API authentication not available')
            return False

        auth_url = bsapi.create_auth_url(self.host, self.client_callback_url, self.app_id, self.app_key)
        ...
        print('Afterwards it will redirect you to a URL that looks similar to the following:')
        print()
        print(f'{self.client_callback_url}&x_a=<user_id>&x_b=<user_key>&x_c=...')
        print()
        print('Once you obtain this URL, paste it in the input below, or type "cancel" to cancel')
        print('Note that this page may not load correctly, or is immediately redirected')
    ...
    def _test_identity(self, identity: Identity):
        ...
        context = bsapi.APIContext(self.app_id, self.app_key, identity.user_id, identity.user_key, self.host)
        api = bsapi.BSAPI(context, '', '')
    ...
    def get_identity(self, tag: str = None) -> Optional[Identity]:
        ...
        while True:
            ...
            self._list_identities()
            print()
            print('Select an identity to use, or start the manager to add/modify identities')
            print()
            ...
            if command == 'select':
                ...
                if identity:
                    if tag:
                        confirm = input(
                            f'Do you always want to use this identity for tag "{tag}"? [yes/no]: ').strip().lower()
                        if confirm in ['y', 'yes']:
                            self.store.set_default(self.host, tag, identity)
                            self.save_store()
                    return identity
