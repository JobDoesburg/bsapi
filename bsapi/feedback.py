...
class BasicCodeEncoder(FeedbackEncoder):
    ...
    VALID_LANGUAGES = ['cpp', 'csharp', 'markup', 'java', 'javascript', 'python', 'arduino', 'armasm', 'bash', 'c',
                       'clike', 'css', 'haskell', 'json', 'kotlin', 'latex', 'matlab', 'plain', 'r', 'racket', 'regex',
                       'sql', 'wolfram']
    _INLINE_CODE_REGEX = r'`([^`\n]+)`'

    def __init__(self, default_language: str = 'cpp', line_numbers: bool = True, dark_theme: bool = False):
        ...
        if dark_theme: code_block_classes.append('d2l-code-dark')
        if line_numbers: code_block_classes.append('line-numbers')
    ...
    def _encode_codeblock(self, code: str) -> str:
        ...
        if language not in self.VALID_LANGUAGES:
            logger.warning(f'unknown code block language "{language}", considering it as code')
            code = language + '\n' + code
        ...
        while i < len(feedback):
            ...
            if idx_code == 0:
                ...
                if idx_end == -1:
                    ...
                    logger.warning('unclosed code block, considering it as regular feedback')
